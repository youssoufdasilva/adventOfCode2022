...
public class Puzzle1 {
    ...
    public static int[] getTargetCoords (String target, ArrayList<String> map) {
        ...
        while (myRowIterator < map.size()) {
            ...
            while (myColumnIterator < map.get(myRowIterator).length()) {
                if (Character.toString( map.get(myRowIterator).charAt(myColumnIterator) ).equals(target)) {
                    targetCoords[0] = myRowIterator;
                    targetCoords[1] = myColumnIterator;
                    return targetCoords;
                }
                myColumnIterator++;
            }
    ...
    public static void main(String[] args) {
        ...
        int[] start = getTargetCoords("S", heightmap);
        int[] end = getTargetCoords("E", heightmap);
                            ...
                            if (col >= 0 && col < heightmap.get(row).length()) {
                                ...
                                if (visited[destination[0]][destination[1]] == false) {
                                    ...
                                    if (newAscii == currentAscii || newAscii == currentAscii+1 || newAscii < currentAscii ) {
                                        ...
                                        if ((currentElevation.equals("y") || currentElevation.equals("z") ) && newElevation.equals("E")) {
                                            System.out.println("\n\t\t\t\t\t Found it! \n");
