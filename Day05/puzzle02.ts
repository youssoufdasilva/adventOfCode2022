...
function moveCrates(numOfCrates: number, fromStack: Array<string>, toStack: Array<string>): Array<Array<string>> {
    if (fromStack.length < numOfCrates) return [fromStack, toStack];
    else {
        let items: Array<string> = fromStack.splice(fromStack.length - numOfCrates, numOfCrates);
...
for (const procedure of allProcedures) {
    ...
    const moveResult: Array<Array<string>> = moveCrates(numOfMoves, finalCrateArrangement[from], finalCrateArrangement[to]);
